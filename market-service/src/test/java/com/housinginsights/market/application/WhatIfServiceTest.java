...
class WhatIfServiceTest {
    ...
    void sendsOneOrderedBatchAndCalculatesDifferences() {
        CapturingPredictionClient client =
                new CapturingPredictionClient(List.of(200000L, 250000L, 150000L));
        ...
        assertThat(client.calls.getFirst())
                .containsExactly(baseline, scenarios.get(0), scenarios.get(1));
        assertThat(result.baselinePrediction()).isEqualTo(200000);
        assertThat(result.scenarios().get(0).priceDifference()).isEqualTo(50000);
        assertThat(result.scenarios().get(0).percentageDifference())
    ...
    void zeroBaselineReturnsNullPercentage() {
        WhatIfService service = new WhatIfService(
                properties -> List.of(0L, 100L)
        );

        WhatIfResult result = service.compare(
                features(1500),
                List.of(features(1800))
        );
    ...
    private static final class CapturingPredictionClient
            implements PredictionClient {
