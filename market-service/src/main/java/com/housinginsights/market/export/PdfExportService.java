...
public class PdfExportService {
    ...
    public byte[] export(MarketFilter filter, MarketAnalysis analysis) {
        try (var document = new PDDocument();
            ...
            for (MarketChartRenderer.RenderedChart chart
                    : chartRenderer.render(analysis)) {
    ...
    private static void addSummaryPage(
            PDDocument document,
            MarketFilter filter,
        ...
        try (var content = new PDPageContentStream(document, page)) {
            ...
            y -= 8;
            y = line(content, BODY_FONT, 11, 45, y,
                    "Matching properties: " + analysis.count());
            ...
            y = line(content, BODY_FONT, 11, 55, y,
                    "Minimum: " + summary.minimum());
            y = line(content, BODY_FONT, 11, 55, y,
                    "Maximum: " + summary.maximum());
            y = line(content, BODY_FONT, 11, 55, y,
                    "Average: " + summary.average());
            line(content, BODY_FONT, 11, 55, y,
    ...
    private static void addChartPage(
            PDDocument document,
            MarketChartRenderer.RenderedChart chart
    ) throws IOException {
        PDPage page = new PDPage(PDRectangle.A4);
        document.addPage(page);
    ...
    private static List<String> describe(MarketFilter filter) {
        ...
        addRange(values, "Square footage", filter.minSquareFootage(),
        ...
        addRange(values, "Distance to city center",
                filter.minDistanceToCityCenter(),
                filter.maxDistanceToCityCenter());
        addRange(values, "School rating", filter.minSchoolRating(),
    ...
    private static void addRange(
        ...
        if (minimum != null || maximum != null) {
            values.add(
                    label + ": "
                            + (minimum == null ? "any" : minimum)
                            + " to "
                            + (maximum == null ? "any" : maximum)
            );
        }
    }
}
