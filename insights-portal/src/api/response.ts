...
import { errorResponseSchema, type ErrorResponse } from "@/api/types";
...
export function dependencyError(dependency: Dependency): ErrorResponse {
  return {
...
export function invalidResponseError(dependency: Dependency): ErrorResponse {
  return {
...
export function normalizeErrorResponse(candidate: unknown, dependency: Dependency): ErrorResponse {
  const result = errorResponseSchema.safeParse(candidate);
  return result.success ? result.data : dependencyError(dependency);
}

export async function parseResponseJson<T>(response: Response, schema: ZodType<T>): Promise<T> {
