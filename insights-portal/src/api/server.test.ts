...
describe("server backend client", () => {
  ...
  it("preserves backend status and error details", async () => {
    const fetchMock = vi.fn(() =>
      Promise.resolve(jsonResponse({ error_code: "invalid_request", message: "Invalid input" }, { status: 422 })),
    );
  ...
  it("maps a network failure to a safe dependency error", async () => {
    ...
    await expect(backend.getMarketAnalysis()).rejects.toMatchObject({
      status: 503,
      body: {
  ...
  it("keeps the timeout active while reading the response body", async () => {
    vi.useFakeTimers();
    const fetchMock = vi.fn((_input: string, init: RequestInit | undefined) =>
      Promise.resolve({
        ok: true,
        status: 200,
        headers: new Headers(),
