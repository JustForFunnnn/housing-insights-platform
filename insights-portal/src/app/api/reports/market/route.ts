...
import { getMarketAnalysis, getMarketMetadata } from "@/api/server";
...
export async function GET(request: NextRequest) {
  try {
    ...
    const [analysis, metadata] = await Promise.all([getMarketAnalysis(filters.toString()), getMarketMetadata()]);
    const buffer = await renderMarketReport(analysis, metadata, filters.toString().replaceAll("&", ", "));
