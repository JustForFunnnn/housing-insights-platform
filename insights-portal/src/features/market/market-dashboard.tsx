...
import type { MarketAnalysis, MarketMetadata, PropertyPage } from "@/api/types";
...
export function MarketDashboard({
  ...
  const { analysis, properties, current, applyFilters, resetFilters, sort, page } = useMarketDashboard({
    ...
    <>
      ...
      {analysis.isError ? (
        <div style={{ marginTop: 24 }}>
          <ErrorNotice error={toApiError(analysis.error)} onRetry={() => analysis.refetch()} />
        </div>
        ...
        <>
          <section aria-label="Market summary" style={{ marginTop: 28 }}>
            <div className="metric-grid">
              <div className="metric">
                ...
                <strong className="metric-value">{analysis.data.count}</strong>
              </div>
              {[
          ...
          <MarketCharts analysis={analysis.data} unit={initialMetadata.price_currency} />
        </>
      ) : (
      ...
      <section style={{ marginTop: 34 }} aria-busy={properties.isFetching}>
        ...
        {properties.isError ? (
          <ErrorNotice error={toApiError(properties.error)} onRetry={() => properties.refetch()} />
        ) : properties.data ? (
