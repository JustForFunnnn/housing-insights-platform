...
const marketServiceUrl = process.env.MARKET_SERVICE_URL ?? "http://localhost:9002";
...
test("moves between both applications", async ({ page }) => {
  ...
  await expect(page.getByLabel(/Interior area/)).toHaveAttribute("min", "1");
...
test("compares properties and exposes the saved history", async ({ page }) => {
  ...
  await expect(page.getByRole("heading", { name: "Previous model readings." })).toBeVisible();
  await expect(
    page.getByRole("table", {
...
test("keeps market filters, sorting and pagination in the URL", async ({ page }) => {
  ...
  await expect(page).toHaveURL(new RegExp(`bedrooms=${selectedBedroom}.*offset=0`));
...
test("runs what-if validation and displays signed differences", async ({ page }) => {
  await page.goto("/market/what-if");
  const baselineBedrooms = page.locator('section[data-coordinate^="BASE"]').getByLabel("Bedrooms");
  await baselineBedrooms.fill("2.5");
  await page.getByRole("button", { name: "Run what-if" }).click();
  await expect(page.getByText("Bedrooms must be a whole number.")).toBeVisible();
  ...
  await expect(featureSelects.nth(1).locator('option[value="square_footage"]')).toHaveCount(0);
  ...
  await scenario.getByLabel("New school rating for scenario 1").fill("8.5");
  await scenario.getByLabel("New bedrooms for scenario 1").fill("4");

  await page.getByRole("button", { name: "Run what-if" }).click();
  await expect(page.getByText("Scenario readings")).toBeVisible();
  await expect(page.getByRole("heading", { name: "Scenario prices" })).toBeVisible();
  await expect(page.getByRole("heading", { name: "Difference from baseline" })).toHaveCount(0);
  const priceBars = page.locator(".what-if-price-chart path.what-if-price-bar");
  ...
  await expect(tooltip.getByText("Percentage", { exact: true })).toBeVisible();
  await expect(page.getByRole("table", { name: "What-if scenario results" })).toBeVisible();
});

test("exports the current market segment as CSV and PDF", async ({ request }) => {
  ...
  const pdf = await request.get("/api/reports/market?bedrooms=3");
  expect(pdf.ok()).toBeTruthy();
  expect(pdf.headers()["content-type"]).toContain("application/pdf");
  expect(pdf.headers()["content-disposition"]).toContain("market-analysis.pdf");
  expect((await pdf.body()).subarray(0, 4).toString()).toBe("%PDF");
});

test("supports keyboard navigation through the application rail", async ({ page }) => {
  await page.goto("/");
  await page.keyboard.press("Tab");
  await expect(page.getByRole("link", { name: "Housing Insights home" })).toBeFocused();
  await page.keyboard.press("Tab");
  await page.keyboard.press("Tab");
  await expect(page.getByRole("link", { name: "Estimator", exact: true })).toBeFocused();
